import { BOMB_3X3, BOMB_4X4, BOMB_5X5, BOMB_6X6 } from "./FontAwesomeSource";
import {
  PLANE,
  PLANE_ARRIVAL,
  PLANE_DEPARTURE,
...
export default (horizontalNum) => {
  ...
  while (stageArray.length < answer * 2) {
    ...
    const stageContentFunc = sourceArray[randomIndex];
    const stageContent = stageContentFunc(horizontalNum);

    /* every - 모든 요소가 true일 때만 true 리턴 */
    const checkNew = stageArray.every((item) => item.key !== stageContent.key);
